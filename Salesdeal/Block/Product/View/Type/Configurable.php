...
class Tejar_Catalog_Block_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Type_Configurable 
{
    ...
    public function getJsonConfig()
    {
        ...
        foreach ($this->getAllowProducts() as $product) {
			...
			//Collect SKU(s) for Associated products in an array
			$productSkuArray[] = $product->getSku();
			
			$mainProductArray = $this->getAllowProducts();
        ...
        foreach ($this->getAllowAttributes() as $attribute) {
            ...
            if (is_array($prices)) {
                foreach ($prices as $value) {
					...
					//ZEE CODE ==== populate sku values into info[options] ===== //
					$skuCount = 0;
					foreach($info['options'] as $infoOptionSku){
						
							$info['options'][$skuCount]['sku'] = $productSkuArray[$skuCount];
							$skuCount++;
						}
		...
		//echo "ddddddddddddddddddddddddd";die;
