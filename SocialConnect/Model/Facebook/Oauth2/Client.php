...
class Tejar_SocialConnect_Model_Facebook_Oauth2_Client
{
    const REDIRECT_URI_ROUTE = 'socialconnect/facebook/connect';

    const XML_PATH_ENABLED = 'customer/tejar_socialconnect_facebook/enabled';
    const XML_PATH_CLIENT_ID = 'customer/tejar_socialconnect_facebook/client_id';
    const XML_PATH_CLIENT_SECRET = 'customer/tejar_socialconnect_facebook/client_secret';
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                ...
                throw new Tejar_SocialConnect_Model_Facebook_Oauth2_Exception($message);
            } else {
                $message = sprintf(
                    Mage::helper('tejar_socialconnect')
