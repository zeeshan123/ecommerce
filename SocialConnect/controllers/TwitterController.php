...
class Tejar_SocialConnect_TwitterController extends Tejar_SocialConnect_Controller_Abstract
{

    public function requestAction()
    {
        $client = Mage::getSingleton('tejar_socialconnect/twitter_oauth_client');
        if(!($client->isEnabled())) {
            $this->norouteAction();
        }
    ...
    protected function _disconnectCallback(Mage_Customer_Model_Customer $customer) {
        Mage::helper('tejar_socialconnect/twitter')->disconnect($customer);
    ...
    protected function _connectCallback() {
        ...
        $info = Mage::getModel('tejar_socialconnect/twitter_info')->load();

        $token = $info->getClient()->getAccessToken();

        $customersByTwitterId = Mage::helper('tejar_socialconnect/twitter')
        ...
        if(Mage::getSingleton('customer/session')->isLoggedIn()) {
            ...
            Mage::helper('tejar_socialconnect/twitter')->connectByTwitterId(
                $customer,
                $info->getId(),
                $token
            );
        ...
        if($customersByTwitterId->getSize()) {
            ...
            Mage::helper('tejar_socialconnect/twitter')->loginByCustomer($customer);
        ...
        $customersByEmail = Mage::helper('tejar_socialconnect/twitter')
        ...
        Mage::helper('tejar_socialconnect/twitter')->connectByCreatingAccount(
            $info->getEmail(),
            $info->getName(),
            $info->getId(),
            $token
